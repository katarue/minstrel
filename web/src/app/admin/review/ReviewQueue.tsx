...
function DetailPanel({
  ...
  return (
    <div className="p-5 flex flex-col h-full gap-4">
      ...
      {/* 日時未取得時: 公式URLを入力して採用時に自動補完 */}
      {needsUrl && (!result || result.type === "error") && (
        <div className="flex flex-col gap-1.5 border border-gold/30 rounded-md px-3 py-3 bg-parchment-dark/20">
          <label className="block text-xs text-ink-body/40">公式ページURL（採用時に日時・会場を自動補完）</label>
          <input
            type="url"
          ...
          />
          <p className="text-xs text-ink-body/35">ツイートに載っている公式ページURLを貼り付けてください（任意）</p>
        </div>
      )}
