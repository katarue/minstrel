...
export default async function TitlesPage() {
  ...
  try {
    ...
    if (error) {
      console.error("Failed to fetch game_titles:", error);
    } else {
      titles = (data ?? []) as GameTitleRow[];
    }
  } catch (err) {
            ...
            <Link key={title.id} href={`/titles/${title.id}`} className="block group">
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  {title.series_name && (
                    <p className="font-body text-ink-body/60 text-sm">{title.series_name}</p>
                  )}
                  {title.publisher && (
                    <p className="font-body text-ink-body/50 text-sm">{title.publisher}</p>
                  )}
                </div>
              </div>
            </Link>
