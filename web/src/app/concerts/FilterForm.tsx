...
const GENRES = [
  ...
  { value: "orchestra", label: "オーケストラ" },
  { value: "wind", label: "吹奏楽" },
  { value: "rock", label: "バンド" },
  { value: "acoustic", label: "アコースティック" },
  { value: "chamber", label: "室内楽" },
...
export default function FilterForm({ q: initQ, prefecture: initPref, genre: initGenre, period: initPeriod }: Props) {
  ...
  return (
    ...
    >
      ...
      <select
        value={period}
        onChange={(e) => { setPeriod(e.target.value); push({ period: e.target.value }); }}
      ...
      <select
        value={prefecture}
        onChange={(e) => { setPrefecture(e.target.value); push({ prefecture: e.target.value }); }}
        className={selectClass}
      >
      ...
      <select
        value={genre}
        onChange={(e) => { setGenre(e.target.value); push({ genre: e.target.value }); }}
        className={selectClass}
      >
        {GENRES.map((g) => (
