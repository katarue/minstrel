...
type OrganizerRow = { id: string; name: string; official_site_url: string | null; region: string | null };
          ...
          {organizers.map((org) => (
            <Link key={org.id} href={`/organizers/${org.id}`} className="block group">
              <div className="bg-parchment-dark rounded-md p-6 h-full transition-all duration-200 ease-in-out group-hover:-translate-y-1 group-hover:shadow-[0_4px_16px_rgba(59,47,29,0.18)]"
                ...
                <h2 className="font-heading text-ink-heading text-lg font-semibold leading-snug mb-3">{org.name}</h2>
                <div className="flex flex-col gap-1">
                  {org.region && <p className="font-body text-ink-body/60 text-sm">{org.region}</p>}
                  {org.official_site_url && (
                    <p className="font-body text-bordeaux text-sm">{t("officialSite")} ↗</p>
                  )}
                </div>
              </div>
            </Link>
