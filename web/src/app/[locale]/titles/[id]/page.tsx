...
type GameTitleDetail = { id: string; title_name: string; series_name: string | null; publisher: string | null };
...
export default async function TitleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: titleData, error: titleError } = await supabase
    .from("game_titles").select("id, title_name, series_name, publisher").eq("id", id).single();
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 md:px-8 py-12">
      <Link href="/titles" className="inline-flex items-center gap-1 font-body text-bordeaux hover:text-bordeaux/70 transition-colors text-sm mb-8">
        {t("backToList")}
      </Link>
      ...
      <div className="bg-parchment-dark rounded-md p-6 md:p-8 flex flex-col gap-4 mb-8"
        ...
        <h1 className="font-heading text-ink-heading text-2xl md:text-3xl font-bold leading-snug">{title.title_name}</h1>
            ...
            {events.map((event) => (
              <Link key={event.id} href={`/events/${event.id}`} className="block group">
                <div className="bg-parchment-dark rounded-md p-4 md:p-5 transition-all duration-200 ease-in-out group-hover:-translate-y-0.5 group-hover:shadow-[0_4px_16px_rgba(59,47,29,0.18)]"
                  style={{ boxShadow: "0 2px 8px rgba(59, 47, 29, 0.12)" }}>
                  <p className="font-heading text-ink-heading text-base font-semibold leading-snug mb-1">{event.event_name}</p>
