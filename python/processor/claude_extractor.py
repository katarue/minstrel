...
【game_titles の判定基準】
game_titles には「ビデオゲーム・コンピューターゲームが最初の発表媒体であるタイトル」のゲーム作品名のみを含めてください。
- 含める: RPG、アクション、ADV、ビジュアルノベル、ノベルゲーム等すべてのゲームジャンルのゲーム作品名
...
- 【重要】game_titles に入れるのは「ゲーム作品名（フランチャイズ名）」であり、「曲名・楽曲名」は絶対に含めない。
    NG例（曲名）:「キャラバンの到着」「Vector to the Heavens」「彼方の光」「序曲」「愛のテーマ」
    OK例（作品名）:「ドラゴンクエスト」「ファイナルファンタジーXIV」「ファイナルファンタジー」
    → 曲名だけが分かってゲーム作品名が特定できない場合は game_titles に含めない
- タイトルの粒度は以下のルールに従うこと:
...
EXTRACTION_SCHEMA = {
    ...
    "game_titles": ["ゲーム作品名（フランチャイズ名）のみ。曲名・楽曲名は絶対に含めない。例:「ドラゴンクエスト」「ファイナルファンタジーXIV」"],
